	...
	f.Describe("L7 Reverse Proxy", func(s *btesting.Scenario) {
		...
		s.Context("authorization flow", func(s *btesting.Scenario) {
			...
			s.Step("request backend url", func(s *btesting.Scenario) {
				...
				s.It("should redirect to entry point", func(m *btesting.Matcher) bool {
			...
			s.Step("enter authorization flow", func(s *btesting.Scenario) {
				...
				s.It("should redirect to OpenID Connect auth endpoint", func(m *btesting.Matcher) bool {
				...
				s.It("receive the cookie", func(m *btesting.Matcher) bool {
			...
			s.Step("show authorization view of identity provider", func(s *btesting.Scenario) {
				...
				s.It("should get a page", func(m *btesting.Matcher) bool {
			...
			s.Step("login identity provider", func(s *btesting.Scenario) {
				...
				s.It("should redirect to callback", func(m *btesting.Matcher) bool {
					m.StatusCode(http.StatusFound)
					u, err := m.LastResponse().Location()
					m.NoError(err)
			...
			s.Step("follow redirect", func(s *btesting.Scenario) {
				...
				s.It("should redirect to backend", func(m *btesting.Matcher) bool {
					m.StatusCode(http.StatusFound)
					u, err := m.LastResponse().Location()
					m.NoError(err)
		...
		s.Context("access to the unknown backend", func(s *btesting.Scenario) {
			s.Context("by authenticated user", func(s *btesting.Scenario) {
				...
				s.It("should close connection", func(m *btesting.Matcher) bool {
		...
		s.Context("access to the backend", func(s *btesting.Scenario) {
			...
			s.Context("by unauthenticated agent", func(s *btesting.Scenario) {
				...
				s.It("should redirect to IdP", func(m *btesting.Matcher) bool {
					m.StatusCode(http.StatusSeeOther)
					u, err := m.LastResponse().Location()
					m.NoError(err)
			...
			s.Context("by authenticated user", func(s *btesting.Scenario) {
				s.Context("who allowed an access", func(s *btesting.Scenario) {
					...
					s.It("should proxy to backend", func(m *btesting.Matcher) bool {
				...
				s.Context("who not allowed an access", func(s *btesting.Scenario) {
					...
					s.It("should not proxy to the backend", func(m *btesting.Matcher) bool {
...
func TestL4Proxy(t *testing.T) {
	...
	f.Describe("L4 Proxy", func(s *btesting.Scenario) {
		...
		s.Context("user accesses to backend", func(s *btesting.Scenario) {
			...
			s.AfterAll(func(m *btesting.Matcher) bool {
				tunnel.Stop()
			...
			s.Step("init user env", func(s *btesting.Scenario) {
				...
				s.It("should create new private key", func(m *btesting.Matcher) bool {
					return m.FileExists(filepath.Join(tunnel.Homedir, userconfig.Directory, userconfig.PrivateKeyFilename))
				})

				s.It("should create CSR file", func(m *btesting.Matcher) bool {
			...
			s.Step("send CSR to the dashboard", func(s *btesting.Scenario) {
				...
				s.It("should success", func(m *btesting.Matcher) bool {
			...
			s.Step("import the certificate", func(s *btesting.Scenario) {
				...
				s.It("should create the certificate file", func(m *btesting.Matcher) bool {
			...
			s.Step("open URL for authenticate", func(s *btesting.Scenario) {
				...
				s.It("open URL", func(m *btesting.Matcher) bool {
					return tunnel.OpenURL(m)
				})
			})
			...
			s.Step("authorizing", func(s *btesting.Scenario) {
				...
				s.It("returns status OK", func(m *btesting.Matcher) bool {
					return m.StatusCode(http.StatusOK)
				})
			})
			...
			s.Step("authorized", func(s *btesting.Scenario) {
				...
				s.It("returns status Found", func(m *btesting.Matcher) bool {
					return m.StatusCode(http.StatusFound)
				})
			})
			...
			s.Step("follow redirect", func(s *btesting.Scenario) {
				...
				s.It("pass the token to the local process", func(m *btesting.Matcher) bool {
					return m.FileExists(filepath.Join(tunnel.Homedir, userconfig.Directory, userconfig.TokenFilename))
				})

				s.It("got bytes from TCP Server", func(m *btesting.Matcher) bool {
