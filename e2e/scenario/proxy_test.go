	...
	f.Describe("L7 Reverse Proxy", func(s *btesting.Scenario) {
		...
		s.Context("authorization flow", func(s *btesting.Scenario) {
			...
			s.Step("request backend url", func(s *btesting.Scenario) {
				...
				s.It("should redirect to entry point", func(_ context.Context, m *btesting.Matcher) {
			...
			s.Step("enter authorization flow", func(s *btesting.Scenario) {
				...
				s.It("should redirect to OpenID Connect auth endpoint", func(_ context.Context, m *btesting.Matcher) {
				...
				s.It("receive the cookie", func(_ context.Context, m *btesting.Matcher) {
			...
			s.Step("redirect to login view", func(s *btesting.Scenario) {
				...
				s.It("should get the status code for redirect", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusFound)
				})
			})
			...
			s.Step("show authorization view of identity provider", func(s *btesting.Scenario) {
				...
				s.It("should get a page", func(_ context.Context, m *btesting.Matcher) {
			...
			s.Step("login identity provider", func(s *btesting.Scenario) {
				...
				s.It("should redirect to callback of the auth proxy", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusFound)
					u, err := m.LastResponse().Location()
					m.NoError(err)
			...
			s.Step("follow redirect", func(s *btesting.Scenario) {
				...
				s.It("should redirect to backend", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusFound)
					u, err := m.LastResponse().Location()
					m.NoError(err)
		...
		s.Context("access to the unknown backend", func(s *btesting.Scenario) {
			s.Context("by authenticated user", func(s *btesting.Scenario) {
				...
				s.It("should close connection", func(_ context.Context, m *btesting.Matcher) {
		...
		s.Context("access to the backend", func(s *btesting.Scenario) {
			...
			s.Context("by unauthenticated agent", func(s *btesting.Scenario) {
				...
				s.It("should redirect to IdP", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusSeeOther)
					u, err := m.LastResponse().Location()
					m.NoError(err)
			...
			s.Context("by authenticated user", func(s *btesting.Scenario) {
				s.Context("who allowed an access", func(s *btesting.Scenario) {
					...
					s.It("should proxy to backend", func(_ context.Context, m *btesting.Matcher) {
				...
				s.Context("who not allowed an access", func(s *btesting.Scenario) {
					...
					s.It("should not proxy to the backend", func(_ context.Context, m *btesting.Matcher) {
...
func DescribeL4Proxy(f *framework.Framework) {
	f.Describe("L4 Proxy", func(s *btesting.Scenario) {
		...
		s.Context("user accesses to backend", func(s *btesting.Scenario) {
			...
			s.AfterAll(func(_ context.Context, _ *btesting.Matcher) {
				tunnel.Stop()
				f.Proxy.ClearConf()
			})
			...
			s.Step("init user env", func(s *btesting.Scenario) {
				...
				s.It("should create new private key", func(_ context.Context, m *btesting.Matcher) {
					m.FileExists(filepath.Join(tunnel.Homedir, userconfig.Directory, userconfig.PrivateKeyFilename))
				})

				s.It("should create CSR file", func(_ context.Context, m *btesting.Matcher) {
			...
			s.Step("import the certificate", func(s *btesting.Scenario) {
				s.Subject(func(ctx context.Context, m *btesting.Matcher) {
					tunnel.LoadCert(tunnel.GetFirstCertificate(ctx, m, f.Agents.RPCClient("test@f110.dev")))
				})

				s.It("should create the certificate file", func(_ context.Context, m *btesting.Matcher) {
			...
			s.Step("open URL for authenticate", func(s *btesting.Scenario) {
				...
				s.It("open URL", func(_ context.Context, m *btesting.Matcher) {
					tunnel.OpenURL(m)
				})
			})
			...
			s.Step("authorizing", func(s *btesting.Scenario) {
				...
				s.It("returns status OK", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusOK)
				})
			})
			...
			s.Step("authorized", func(s *btesting.Scenario) {
				...
				s.It("returns status Found", func(_ context.Context, m *btesting.Matcher) {
					m.StatusCode(http.StatusFound)
				})
			})
			...
			s.Step("follow redirect", func(s *btesting.Scenario) {
				...
				s.It("pass the token to the local process", func(_ context.Context, m *btesting.Matcher) {
					m.FileExists(filepath.Join(tunnel.Homedir, userconfig.Directory, userconfig.TokenFilename))
				})

				s.It("got bytes from TCP Server", func(_ context.Context, m *btesting.Matcher) {
