...
func TestProxyController(t *testing.T) {
	...
	f.Describe("ProxyController", func(s *btesting.Scenario) {
		...
		s.BeforeAll(func(m *btesting.Matcher) { f.Proxy.Setup(m, testUserId) })
		...
		s.Step("send a request", func(s *btesting.Scenario) {
			...
			s.It("should return status OK", func(m *btesting.Matcher) {
				m.StatusCode(http.StatusOK)
			})

			s.It("should return Server header", func(m *btesting.Matcher) {
		...
		s.Step("send a request without client credential", func(s *btesting.Scenario) {
			...
			s.It("should return status see other", func(m *btesting.Matcher) {
