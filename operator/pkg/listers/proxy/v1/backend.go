...
type BackendLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Backend, err error)
...
// List lists all Backends in the indexer.
func (s *backendLister) List(selector labels.Selector) (ret []*v1.Backend, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Backend))
	})
	return ret, err
}
...
// List lists all Backends in the indexer for a given namespace.
func (s backendNamespaceLister) List(selector labels.Selector) (ret []*v1.Backend, err error) {
...
// Get retrieves the Backend from the indexer for a given namespace and name.
func (s backendNamespaceLister) Get(name string) (*v1.Backend, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("backend"), name)
	}
	return obj.(*v1.Backend), nil
}
